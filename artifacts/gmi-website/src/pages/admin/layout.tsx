...
const navItems = [
 ...
 { href:"/admin/jobs", label:"Jobs", icon:"○" },
  ...
  { href:"/admin/agri-advisory", label:"Agri Advisory", icon:"♣" },
...
export default function AdminLayout({ children }:{ children:React.ReactNode }) {
 ...
 <h1 className="font-display">GMI Admin</h1>
 ...
 key={item.href}
 href={item.href}
 className={`flex items-center gap-3 px-4 py-3 text-sm hover:bg-white/10 transition-colors ${item.href === "/admin/inquiries" ? "relative" :""}`}
 >
 <span className="text-lg">{item.icon}</span>
 {item.label}
 {item.href === "/admin/inquiries" && inquiryCount > 0 && (
 <span className="ml-auto bg-red-500 text-white text-xs font-bold h-5 min-w-[20px] flex items-center justify-center px-1">
 {inquiryCount}
 </span>
 )}
 </a>
