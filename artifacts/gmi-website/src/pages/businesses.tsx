              ...
              <Link key={i} href={`/businesses/${sub.slug}`} className="group block">
                <div className="h-full flex flex-col bg-white border border-border shadow-sm hover:shadow-xl transition-shadow duration-300 ease-in-out overflow-hidden">
                  <div className="relative h-44 overflow-hidden" style={{ transform: 'translateZ(0)' }}>
                    ...
                    <div className="absolute bottom-3 right-3 bg-white/20 backdrop-blur-md text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
                      <ArrowUpRight size={16} />
                    </div>
                  </div>
                  <div className="p-5 flex flex-col flex-1">
                    <h3 className="font-display text-foreground mb-2 group-hover:text-primary transition-colors duration-300 ease-in-out">{sub.name}</h3>
