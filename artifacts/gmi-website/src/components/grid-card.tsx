...
export function GridCard({ icon:Icon, iconColor, title, description, className = "", children }:GridCardProps) {
 return (
 <div className={`group bg-white p-8 border border-transparent card-hover h-full flex flex-col ${className}`}>
 {Icon && (
 <div className="w-14 h-14 bg-muted text-primary flex items-center justify-center mb-6">
 <Icon size={28} />
 </div>
 )}
