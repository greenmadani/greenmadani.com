...
import { Upload, Search, Trash2, Copy, Check, Image as ImageIcon, File as FileIcon, LayoutGrid, List } from "lucide-react";
        ...
        <div className="flex items-center gap-2">
          <div className="flex border rounded-md overflow-hidden">
            <button
              ...
              className={`p-2 ${viewMode === "grid" ? "bg-[#1A5C38] text-white" : "bg-white text-gray-500 hover:bg-gray-100"}`}
              title="Grid view"
            >
            ...
            <button
              ...
              className={`p-2 ${viewMode === "list" ? "bg-[#1A5C38] text-white" : "bg-white text-gray-500 hover:bg-gray-100"}`}
              title="List view"
            >
            ...
            <TableBody>
              {filtered.map((item) => (
                <TableRow key={item.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setViewItem(item)}>
                  <TableCell>
                    {item.mimeType?.startsWith("image/") ? (
                      <img src={item.url} alt={item.filename} className="h-10 w-10 object-cover rounded" />
                    ) : (
                  ...
                  <TableCell><Badge variant="secondary" className="text-xs">{item.mimeType}</Badge></TableCell>
              ...
              {filtered.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-sm text-gray-500 py-12">No media found</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
